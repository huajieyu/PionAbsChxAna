...
void buildIncident(double *incident, int nBin, double initial[], double inter[]){
  ...
  cout << "Building incident histogram...\n";
  for (int i=0; i<=nBin+1; i++){
    cout << "i=" << i << "\tInitial=" << initial[i] << "\tInteracting=" << inter[i] << endl;
  }
  ...
  for(int i=nBin+1; i>0; i--){
    ...
    cout << "Slice " << i << "\tBirth=" << birth << "\tDeath=" << death << endl;
    for(int j=i-1; j>0; j--){
      incident[j] += birth;
    }
    for(int j=i-1; j>0; j--){
    ...
    cout << "Incident in slice " << i << " is " << incident[i] << endl << endl;
  }
  return;
}

double Main::Maker::true_interE_francesca(double endX, double endY, double endZ, double startKE, TH1* runningSum_true_dE){


double dist = sqrt(pow(endX-beam_startX_mc,2) + pow(endY-beam_startY_mc,2) + pow(endZ-beam_startZ_mc,2));
double endKE;
...
void Main::Maker::hist_bethe_mean_distance(double E_init, double mass_particle, TH1D* h_bethe ){
...
void Main::Maker::MakeFile() 
{
  ...
  for(int i=0; i<=eslice_nBin+1; i++){
     ...
     true_initial_eslice_pion_decay[i]=0;
     true_incident_eslice_upstream[i]=0;
     true_incident_eslice_upstream_pion[i]=0;
     true_interacting_eslice_upstream[i]=0;
     true_initial_eslice_upstream[i]=0;
     true_incident_eslice_muon[i]=0;
     ...
     test_add_upstream_inter_inel[i]=0;
     test_add_upstream_init_inel[i]=0;
     test_add_upstream_inter_decay[i]=0;
     test_add_upstream_init_decay[i]=0;


     reco_incident_eslice_pion_inel[i]=0;
     reco_interacting_eslice_pion_inel[i]=0;
     reco_incident_eslice_pion_decay[i]=0;
     reco_interacting_eslice_pion_decay[i]=0;
     reco_incident_eslice_upstream[i]=0;
     reco_interacting_eslice_upstream[i]=0;
     reco_incident_eslice_muon[i]=0;
     reco_interacting_eslice_muon[i]=0; 
  ...
  for(int i= _initial_entry; i<evts; i++){
	...
	//
	true_esliceID = int(true_endKE_eslice/eslice_bin_size) + 1;
	start_true_esliceID = int(true_startKE_eslice/eslice_bin_size) + 1;

	


	//checking that a particle does not interact in the bin it starts in
	if(true_esliceID < 1 ) {true_esliceID = 0;}
	if(start_true_esliceID < 1) {start_true_esliceID = 0;}
	if(true_esliceID > eslice_nBin) {true_esliceID = eslice_nBin+1;}
	if(start_true_esliceID > eslice_nBin) {start_true_esliceID = eslice_nBin+1;} 
	if(esliceID < 1){esliceID = 0;}
	...
	if(t->reco_beam_incidentEnergies->size()==t->reco_beam_calo_wire_z->size()
           ...
           && t->reco_beam_TrkPitch_SCE->size()>0 && t->reco_beam_dEdX_SCE->size()>0){
			...
			//std::cout<<"this_sliceID is "<<this_sliceID<<std::endl;
          		...
          		if (this_sliceID<0 || this_sliceID>=nslices+3) continue;
    ...
    //Yinrui's slicing method//////////////////////////
    double temp = 999;
    int start_idx = 0;
    true_trklen_accum.reserve(t->true_beam_traj_Z->size()); // initialize true_trklen_accum
    for (int i=0; i<t->true_beam_traj_Z->size(); i++){
      if (abs(t->true_beam_traj_Z->at(i)) < temp){
        temp = abs(t->true_beam_traj_Z->at(i));
    ...
    for (int i=start_idx+1; i<t->true_beam_traj_Z->size(); i++){
      if (i == start_idx+1) true_trklen = 0;
      true_trklen += sqrt( pow( t->true_beam_traj_X->at(i)-t->true_beam_traj_X->at(i-1), 2)
                          +pow( t->true_beam_traj_Y->at(i)-t->true_beam_traj_Y->at(i-1), 2)           
                	  +pow( t->true_beam_traj_Z->at(i)-t->true_beam_traj_Z->at(i-1), 2));  
      true_trklen_accum[i] = true_trklen;
      //cout<<i<<"\t"<<true_trklen_accum[i]<<endl;
      }   
      
      // calculate reco track length
      reco_trklen_accum.reserve(t->reco_beam_calo_Z->size());
      reco_trklen = -1999;
      for (int i=1; i<t->reco_beam_calo_Z->size(); i++){
        if (i == 1) reco_trklen = 0;
        reco_trklen += sqrt( pow( (t->reco_beam_calo_X->at(i)-t->reco_beam_calo_X->at(i-1)), 2) 
        	    	    +pow( (t->reco_beam_calo_Y->at(i)-t->reco_beam_calo_Y->at(i-1)), 2)
			    +pow( (t->reco_beam_calo_Z->at(i)-t->reco_beam_calo_Z->at(i-1)), 2));
	...
	double reco_beam_trklen = reco_trklen;
	cout << "Calo endZ: " << t->reco_beam_calo_endZ << ", new sum_pitch: " << sum_pitch_all << ", reco_trklen: " << reco_trklen << endl;
	cout << "True_trklen: " << true_trklen << endl;
	...
	//  				+TMath::Power((t->true_beam_startZ-t->true_beam_endZ),2.0));
        ...
        //true_sliceID = int(t->true_beam_endZ/thinslicewidth); //endpoint z slicing - Libo
	//true_sliceID = int(true_beam_trklen/thinslicewidth);  //slicing by distance between start and endpoints
	true_sliceID = int(true_trklen/thinslicewidth); //Yinrui slicing

	cout << "Trklen Slice ID:\t" << int(reco_beam_trklen/thinslicewidth) << "\ttrue: " << int(true_trklen/thinslicewidth) << endl; 
	...
	if (isInelastic && true_sliceID < nslices){
	...
	//CATEGORY 2 "pion Inelastics"
	   ...
	   if(t->true_beam_endZ > 0 /*true_esliceID >0 && true_esliceID < eslice_nBin+1*/){ //without over/underflow
	   ...
	   if (t->true_daughter_nPi0 == 0 && t->true_daughter_nPiPlus == 0 && t->true_daughter_nPiMinus ==0){//true absorption
	       //if(true_esliceID>=0 && true_esliceID <= eslice_nBin+1){ //with over/underflow bins
	       if(1/*true_esliceID>0 && true_esliceID < eslice_nBin+1*/){ //without over/underflow bins
	...
	if(isPiInelastic && t->true_beam_endZ > 0 /*&& true_esliceID < start_true_esliceID && true_esliceID >0 && true_esliceID < eslice_nBin+1 && start_true_esliceID >= 0  && start_true_esliceID <= eslice_nBin+1*/){ //without over/underflow
	...
	else if(isPiDecay && t->true_beam_endZ > 0/*&& true_esliceID < start_true_esliceID && true_esliceID >0 && true_esliceID < eslice_nBin+1 && start_true_esliceID >= 0  && start_true_esliceID <= eslice_nBin+1*/){ //no over/underflow
	...
	else if(isUpstream && abs(t->true_beam_PDG ==211) && t->true_beam_endZ > 0/*&& true_esliceID < start_true_esliceID && true_esliceID >0 && true_esliceID < eslice_nBin+1 && start_true_esliceID >= 0  && start_true_esliceID <= eslice_nBin+1*/){ //no over/underflow
            ++true_interacting_eslice_upstream[true_esliceID];
	    ++true_initial_eslice_upstream[start_true_esliceID];
	 if(*temp_Ptr0=="pi+Inelastic"){
		++test_add_upstream_inter_inel[true_esliceID];
		++test_add_upstream_init_inel[start_true_esliceID];
	  }
	  else if(*temp_Ptr0=="Decay"){
	    ++test_add_upstream_inter_decay[true_esliceID];
	    ++test_add_upstream_init_decay[start_true_esliceID];
	  }
        }

	if(abs(t->true_beam_PDG)==211 && t->true_beam_endZ > 0/* && true_esliceID < start_true_esliceID && true_esliceID >=0 && true_esliceID <= eslice_nBin+1 && start_true_esliceID >= 0  && start_true_esliceID <= eslice_nBin+1*/){
	      ...
	      continue;
           ...
           if(i<=eslice_nBin+1){
              ...
              //select muon
                   ...
                   if(abs(t->true_beam_PDG) == 211) {
                        ++true_incident_eslice_upstream_pion[i];
                   }
              }
	      if(abs(t->true_beam_PDG==211)){
	...
	/*MM	
	if(abs(t->true_beam_PDG) == 211){
	 for(int i=esliceID; i<=start_esliceID; i++){
	...
	if(isInelastic && isUpstream==false){
	  for(int i=esliceID; i<=start_esliceID; i++){
...
//////////////End Old Beam Quality Cut/////////////
	 ...
	 //#1 vsize cut
	  ...
	  //LOG_NORMAL()<<"End of fill histogram after beam quality cut"<<std::endl;
        ...
        if(isdata==1){
	  //#0 correspond to PDG cut
          if(!data_beam_PID(t->beam_inst_PDG_candidates)) continue;
	  //Yinrui's pre-preselection data cuts (only one reconstructed beam track)
	  if(t->beam_inst_nMomenta != 1 || t->beam_inst_nTracks != 1) continue;
          h_Evttot->Fill(event_weight);
...
if(passCuts == true){
  ...
  double true_incident_eslice_pion_inel2[eslice_nBin+2];
  double true_incident_eslice_pion_total2[eslice_nBin+2];
  double true_incident_eslice_pion_decay2[eslice_nBin+2]; 
  double true_incident_eslice_upstream2[eslice_nBin+2];
 
  for(int i=0;i<eslice_nBin+2;i++){
    true_incident_eslice_pion_inel2[i]=0;
    true_incident_eslice_pion_total2[i]=0;
    true_incident_eslice_pion_decay2[i]=0;
    true_incident_eslice_upstream2[i]=0;
  }
...
*/
  ...
  double tempxsec_eslice_abs[eslice_nBin+2];
  double tempxsec_eslice_chx[eslice_nBin+2];
  double tempxsec_eslice_inel[eslice_nBin+2];

  double avg_incE_eslice[eslice_nBin+2];
  ...
  for (int i = 0; i<=nslices+2; ++i){
     ...
     cout << "Avg_pitch slice " << i << " is " << avg_pitch[i] << endl;

     tempxsec[i] = MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*log((true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i])/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]-true_abs[i]));
     ...
     for(int nb=0; nb<nbinspmom; nb++){
          tempxsec_pmom[i][nb]=(1/1e-27)*MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*h_energetic_pmom_gen[i]->GetBinContent(nb+1)/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]);
     }
     for(int nb=0; nb<nbinspcostheta; nb++){
          tempxsec_pcostheta[i][nb]=(1/1e-27)*MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*h_energetic_pcostheta_gen[i]->GetBinContent(nb+1)/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]);
     }
     for(int nb=0; nb<nbinspphi; nb++){
          tempxsec_pphi[i][nb]=(1/1e-27)*MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*h_energetic_pphi_gen[i]->GetEntries()/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]);
     }
     if(avg_pitch[i]>0.0001 && true_incident_pion[i]+true_incident_pion_decay[i]>0 && true_chx[i]>0){
     tempxsec_chx[i] = MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*log((true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i])/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]-true_chx[i]));
     ...
     tempxsec_inel[i] = MAr/(Density*NA*thinslicewidth)*log(true_incident[i]/(true_incident[i]-true_interaction[i]))*1e27;

     cout << "i: " << i <<  " tempxsec_inel[i]: " << tempxsec_inel[i] << endl;

     cout << "True_incident: " << true_incident[i] << ", Sum of 3: " <<  true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i] << endl; 

  
     exs_abs[i] = MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*1e27*sqrt(true_abs[i]+pow(true_abs[i],2)/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]))/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]);
     exs_chx[i] = MAr/(Density*NA*thinslicewidth/*avg_pitch[i]*/)*1e27*sqrt(true_chx[i]+pow(true_chx[i],2)/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]))/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]);
     exs_inel[i] = MAr/(Density*NA*thinslicewidth/*10*/)*1e27*sqrt(true_interaction[i]+pow(true_interaction[i],2)/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]))/(true_incident_pion[i]+true_incident_pion_decay[i]+true_incident_upstream_pion[i]); 
  }//end of loop over all the nslices

  //ESlice Method ///////////////////////////////
  double betheMass = PionMass;
  for(int i=1; i<=eslice_nBin;i++){
    //Event-by-event
    double temp_incident_old = true_incident_eslice_pion_inel[i]+true_incident_eslice_pion_decay[i]+true_incident_eslice_upstream_pion[i];
    //Average using buildIncident
    double temp_incident2 = true_incident_eslice_pion_inel2[i]+true_incident_eslice_pion_decay2[i]+true_incident_eslice_upstream2[i];
    double temp_incident = true_incident_eslice_pion_total2[i];

    //cout << "i=" << i << " Incident: Total: " << temp_incident << ", Inel+Decay+Upstream: " << temp_incident2 << endl;
    cout << "i=" << i << " Inel. Inter.:  Total: " << true_interacting_eslice_pion_totinel[i] << ", Upstream+non-upstream: " << true_interacting_eslice_pion_inel[i]+test_add_upstream_inter_inel[i] << endl;
    cout << "i=" << i << " Inel. Inter.: Total: " << true_interacting_eslice_pion_totinel[i] << ", Abs.+Chx.+Rea.: " << true_abs_eslice[i]+true_chx_eslice[i]+true_rea_eslice[i] << endl;
   ...
   //tempxsec_eslice_inel[i] = MAr/(Density*NA*eslice_bin_size)*betheBloch(eslice_bins[i],betheMass)*log((1.0*temp_incident2)/(1.0*(temp_incident2-(true_interacting_eslice_pion_inel[i]+test_add_upstream_inter_inel[i])))); //OLD calulation
    ...
    if((true_incident_eslice_pion_inel[i]-true_abs_eslice[i]) > 0 && true_incident_eslice_pion_inel[i] > 0){
      tempxsec_eslice_abs[i] = MAr/(Density*NA*eslice_bin_size)*betheBloch(eslice_bins[i],betheMass)*log((1.0*temp_incident2)/(1.0*(temp_incident2-true_abs_eslice[i])));}
    ...
    if((true_incident_eslice_pion_inel[i]-true_chx_eslice[i]) > 0 && true_incident_eslice_pion_inel[i] > 0){
      tempxsec_eslice_chx[i] = MAr/(Density*NA*eslice_bin_size)*betheBloch(eslice_bins[i],betheMass)*log((1.0*temp_incident2)/(1.0*(temp_incident2-true_chx_eslice[i])));}
  ...
  */
  ...
  //eslice xsec TGraphs
  TGraph *gr_tempxsec_eslice_inel = new TGraph(eslice_nBin+2,eslice_bins,tempxsec_eslice_inel);
  TGraph *gr_tempxsec_eslice_abs = new TGraph(eslice_nBin+2,eslice_bins,tempxsec_eslice_abs);
  TGraph *gr_tempxsec_eslice_chx = new TGraph(eslice_nBin+2,eslice_bins,tempxsec_eslice_chx);

  TGraph *gr_incident_eslice = new TGraph(eslice_nBin+2,eslice_bins,true_incident_eslice_pion_inel);
  TGraph *gr_inelastic_eslice = new TGraph(eslice_nBin+2,eslice_bins,true_interacting_eslice_pion_inel); 
  TGraph *gr_absorption_eslice = new TGraph(eslice_nBin+2,eslice_bins,true_abs_eslice); 
  TGraph *gr_chx_eslice = new TGraph(eslice_nBin+2,eslice_bins,true_chx_eslice); 
