...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TFile *input(0);
   TString fname = "output_test.root";
   ...
   TTree *signalTree     = (TTree*)input->Get("TreeSignal");
   TTree *background     = (TTree*)input->Get("TreeBackground");
   ...
   //dataloader->AddVariable( "myvar1 := ShwEng+ShwDist", 'F' );
   //dataloader->AddVariable( "myvar2 := ShwEng-ShwDist", "Expression 2", "", 'F' );
   dataloader->AddVariable( "ShwEng", "Variable_1", "units", 'F' );
   dataloader->AddVariable( "ShwDist", "Variable_2", "units", 'F' );
   dataloader->AddVariable( "TrackScore",                "Variable_3", "units", 'F' );
   dataloader->AddVariable( "Chi2",                "Variable_4", "units", 'F' );

   // You can add so-called "Spectator variables", which are not used in the MVA training,
   // but will appear in the final "TestTree" produced by TMVA. This TestTree will contain the
   // input variables, the response values of all trained MVAs, and the spectator variables

   //dataloader->AddSpectator( "spec1 := ShwEng*2",  "Spectator 1", "units", 'F' );
   //dataloader->AddSpectator( "spec2 := ShwEng*3",  "Spectator 2", "units", 'F' );


   // global event weights per tree (see below for setting event-wise weights)
   Double_t signalWeight     = 1.5;
   Double_t backgroundWeight = 1.5;
